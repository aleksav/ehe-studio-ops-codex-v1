...
14. Assign first 2--3 issues to each developer.
15. Review PRs promptly, leave comments, assign next issues as previous close.
16. Verify every PR has web and mobile parity.
17. Create QA issue immediately after each feature PR merges.
18. Monitor E2E CI continuously — red equals build break.
19. Do not release slice N+1 issues until slice N is fully merged and stable.
...
```bash
git commit --author="team-lead-agent <aleksa@tsf.tech>" -m "chore: ..."
git commit --author="developer-a-agent <aleksa@tsf.tech>" -m "feat: ..."
git commit --author="developer-b-agent <aleksa@tsf.tech>" -m "feat: ..."
git commit --author="qa-engineer-agent <aleksa@tsf.tech>" -m "test: ..."
git commit --author="security-expert-agent <aleksa@tsf.tech>" -m "fix: ..."
git commit --author="performance-expert-agent <aleksa@tsf.tech>" -m "perf: ..."
```
...
- The agent role must be encoded in the author name (for example `team-lead-agent <aleksa@tsf.tech>`) so it is immediately visible in `git log` which agent produced each commit.
