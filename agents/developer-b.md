...
- `git commit --author="developer-b-agent <aleksa@tsf.tech>" -m "feat: description #NNN"` -- always use your agent author identity, reference issue in every commit.
...
- Always use your own `--author` string — never the global git config identity. Use the role-specific author name with the shared GitHub email so `git log` clearly shows which agent produced each commit.
